...
const int SUNGEN = (NCOLOR*NCOLOR-1);

const double ROOT2 = 1.41421356237309504880168872421;

int posmat[NCOLOR][NCOLOR] ;
Complex genmat[SUNGEN][NCOLOR][NCOLOR] ;

Complex tr[SUNGEN][SUNGEN], strconst[SUNGEN][SUNGEN][SUNGEN] ;


void initgen(void) ;
void vectomat(Complex [SUNGEN], Complex [NCOLOR][NCOLOR]) ;
...
Complex fund[NCOLOR][NCOLOR] ; 
  ...
  for(aa=0;aa<SUNGEN;aa++) {
    ...
    for(ii=0;ii<SUNGEN;ii++) adj[ii] = Complex(0.0,0.0) ;
...
void vectomat(Complex vec[SUNGEN], Complex mat[NCOLOR][NCOLOR])
{
int ii,jj ;
Complex mult ;
