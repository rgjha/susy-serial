    ...
    Complex *bn, Complex *soln)
{
  ...
  cusp::array1d<cusp::complex<double>, cusp::device_memory> x_d(LEN, 1);

  // monitor the cg iteration silently
  cusp::default_monitor<cusp::complex<double> > monitor(b_d, LEN, 1e-10);
  
  // Solve
  cusp::krylov::cg(AA,x_d,b_d,monitor);
  
  cusp::array1d<cusp::complex<double>, cusp::host_memory> x_h = x_d;
 
    for (int i=0; i<LEN; i++)
    {
      soln[i] = Complex(x_h[i].real(),x_h[i].imag());
    }
}
